...
#define LOG_NDEBUG 0 //FIXME: Disable debug
...
// COMPAT: manta stack <-> slsi stack
#ifndef CSC_MEMORY_TYPE
#define CSC_MEMORY_TYPE CSC_MEMORY_DMABUF /* (CSC_MEMORY_USERPTR) */
#endif
#ifndef GRALLOC_USAGE_PRIVATE_CHROMA
#define GRALLOC_USAGE_PRIVATE_CHROMA GRALLOC_USAGE_PRIVATE_0
#endif
#ifndef HAL_PIXEL_FORMAT_EXYNOS_YV12
#define HAL_PIXEL_FORMAT_EXYNOS_YV12 HAL_PIXEL_FORMAT_EXYNOS_YV12_M
#endif
#ifndef HAL_PIXEL_FORMAT_EXYNOS_YCrCb_420_SP
#define HAL_PIXEL_FORMAT_EXYNOS_YCrCb_420_SP HAL_PIXEL_FORMAT_EXYNOS_YCrCb_420_SP_M
#endif
...
int ExynosCameraHWInterface2::InitializeISPChain()
{
    ...
    sprintf(node_name, "%s%d", NODE_PREFIX, FIMC_IS_VIDEO_SEN1_NUM);
    ...
    /* Open ISP */
    memset(&node_name, 0x00, sizeof(char[30]));
    sprintf(node_name, "%s%d", NODE_PREFIX, FIMC_IS_VIDEO_ISP_NUM);
    fd = exynos_v4l2_open(node_name, O_RDWR, 0);
    ...
    /* Open ScalerC */
    memset(&node_name, 0x00, sizeof(char[30]));
    sprintf(node_name, "%s%d", NODE_PREFIX, FIMC_IS_VIDEO_SCC_NUM);
    fd = exynos_v4l2_open(node_name, O_RDWR, 0);
    ...
    /* Open ScalerP */
    memset(&node_name, 0x00, sizeof(char[30]));
    sprintf(node_name, "%s%d", NODE_PREFIX, FIMC_IS_VIDEO_SCP_NUM);
    fd = exynos_v4l2_open(node_name, O_RDWR, 0);
    if (fd < 0) {
    ...
    if(m_cameraId == 0)
        m_camera_info.sensor_id = SENSOR_NAME_S5K4H5;
    else
...
int ExynosCameraHWInterface2::m_jpegCreator(StreamThread *selfThread, ExynosBuffer *srcImageBuf, nsecs_t frameTimeStamp)
{
    ...
    if (m_exynosPictureCSC) {
        ...
        csc_set_src_buffer(m_exynosPictureCSC,
                           (void **)&srcImageBuf->fd.fd, CSC_MEMORY_TYPE);

        csc_set_dst_buffer(m_exynosPictureCSC,
                           (void **)&m_resizeBuf.fd.fd, CSC_MEMORY_TYPE);
        for (int i = 0 ; i < 3 ; i++)
...
int ExynosCameraHWInterface2::m_recordCreator(StreamThread *selfThread, ExynosBuffer *srcImageBuf, nsecs_t frameTimeStamp)
{
    ...
    if (m_exynosVideoCSC) {
        ...
        csc_set_dst_buffer(m_exynosVideoCSC,
            (void **)(&(subParms->svcBuffers[subParms->svcBufIndex].fd.fd)), CSC_MEMORY_TYPE);
...
int ExynosCameraHWInterface2::m_prvcbCreator(StreamThread *selfThread, ExynosBuffer *srcImageBuf, nsecs_t frameTimeStamp)
{
    ...
    if (subParms->format == HAL_PIXEL_FORMAT_YCrCb_420_SP) {
        if (m_exynosVideoCSC) {
            ...
            csc_set_dst_buffer(m_exynosVideoCSC,
                (void **)(&(m_previewCbBuf.fd.fd)), CSC_MEMORY_TYPE);
